...
class BaseConsumeOrderRepository extends BaseRepository
{
    ...
    {
        return $this->query()
            ->where('consume_orders.shop_id', $shop_id)
            ->with('goods')
            ->with('customer')
            ->with('card')
            ->with('dinning_time');
     ...
     * @return mixed
     */
    public function getByShopWithRelationQuery($shop_id, $start_time, $end_time, $dinning_time_id = null, $pay_method = null, $restaurant_user_id= null, $status = [], $departmentOnly = false, $consumeCategoryOnly=false)
    {
        $query = $this->getByShopQuery($shop_id)->select(
            'consume_orders.*',
            'customers.user_name as customer_name',
            'cards.number as card_number',
            'dinning_time.name as dinning_time_name',
